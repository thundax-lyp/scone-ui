...
- Workspace 包版本由根 `package.json` 的 `version` 统一管理；修改版本时运行 `pnpm version:sync`，PR 验证运行 `pnpm version:check`。
...
- Package：`packages/scone-ui/**`、包 README、AI Guide、公共导出、组件实现、组件测试、包构建配置或根构建配置变更时，至少验证 package typecheck、tests、package build；随包文档或发包边界变化时补充 `pack:check`。Package 是 Example 的 workspace 依赖，Package 变更必须同时触发 Example 验证。
- Example：`apps/example/**` 或 Package 变更时，至少验证 example typecheck、example test、example build。
- Docs site：`apps/scone-docs/**`、`docs/**`、根 README、文档站配置或公开文档口径变化时，至少验证 docs typecheck、docs build。
- Full：`package.json`、`pnpm-lock.yaml`、`pnpm-workspace.yaml`、`scripts/**`、全局 TypeScript/Vite/Vitest/ESLint/Prettier 配置或 workflow 变化时，按 Package、Example、Docs site 全部验证。
