...
- PR 标题的 `Type` 和 `scope` 固定沿用 `TODO-RULES.md` 的白名单。
- 一个 PR 标题只能使用一个 `Type(scope)`，不得用多个 type 或多个 scope 共用一个 PR。需要同时覆盖多个独立治理域时，优先拆 PR；确实不可拆时，选择唯一主 `Type(scope)`，并在 `Scope`、`Cross-domain Impact` 和 `Verification Evidence` 中说明其他影响范围。
