...
- `packages/scone-ui/src/` 下除 `packages/scone-ui/src/components/ui/` 之外的 TypeScript / React 代码。
- `packages/scone-ui/src/components/ui/` 作为 vendored shadcn/Radix primitive 源码，整体排除在 lint / format 之外；上层 Scone wrapper 必须承担公共 API、可访问性、测试和类型边界。
- `packages/scone-ui/src/test/`、`packages/scone-ui/src/**/*.test.ts`、`packages/scone-ui/src/**/*.test.tsx`。
- `apps/example/src/` 示例应用代码。
...
| Rule                                          | Gate                                                                                                                                                                |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SCONE_CODE_ARROW_FUNCTION`                   | ESLint `no-restricted-syntax`                                                                                                                                       |
| `SCONE_CODE_NO_CONSOLE`                       | ESLint `no-console`                                                                                                                                                 |
| `SCONE_CODE_NO_EXPLICIT_ANY`                  | ESLint `@typescript-eslint/no-explicit-any`                                                                                                                         |
| `SCONE_CODE_EXPORTED_FUNCTION_BOUNDARY_TYPES` | ESLint `@typescript-eslint/explicit-module-boundary-types`                                                                                                          |
| `SCONE_CODE_UNKNOWN_CATCH_CALLBACK`           | ESLint `@typescript-eslint/use-unknown-in-catch-callback-variable`                                                                                                  |
| `SCONE_CODE_NO_INFERRABLE_LOCAL_TYPES`        | ESLint `@typescript-eslint/no-inferrable-types`                                                                                                                     |
| `SCONE_CODE_NO_UNNECESSARY_TYPE_ARGUMENTS`    | ESLint `@typescript-eslint/no-unnecessary-type-arguments`、`@typescript-eslint/no-unnecessary-type-constraint`、`@typescript-eslint/no-unnecessary-type-parameters` |
| `SCONE_CODE_NO_NESTED_TERNARY`                | ESLint `no-nested-ternary`                                                                                                                                          |
| `SCONE_CODE_HOOKS_RULES`                      | ESLint `eslint-plugin-react-hooks` recommended rules                                                                                                                |
| `SCONE_FORMAT_PRETTIER`                       | `pnpm format:check` / Prettier                                                                                                                                      |
| `SCONE_PUBLIC_ENTRY_ONLY`                     | `packages/scone-ui/src/index.test.ts` and build types output                                                                                                        |
| `SCONE_RECIPE_DOCS_ONLY`                      | `packages/scone-ui/src/index.test.ts`                                                                                                                               |
| `SCONE_TEST_FILTER_NOISY_STDERR`              | Vitest `onConsoleLog` in `vitest.config.ts`                                                                                                                         |

### Code Quality

- `SCONE_CODE_ARROW_FUNCTION`：生产代码和测试中的命名函数默认使用 `const name = (...) => {}`；禁止 `function name(...)` 和普通 `function (...)` 表达式。由 ESLint `no-restricted-syntax` 门禁。`packages/scone-ui/src/components/ui/` 不纳入门禁。
...
- `SCONE_FORMAT_PRETTIER`：全库格式使用 Prettier，缩进为 4 spaces。`packages/scone-ui/src/components/ui/**/*.{ts,tsx}` 作为 vendored primitive 排除在格式化之外。

### Public Boundary

- `SCONE_PUBLIC_ENTRY_ONLY`：公共组件、Pattern、service、hook、helper 和类型只能从 `packages/scone-ui/src/index.ts` 形成发布入口；`packages/scone-ui/src/components/ui/*` 不作为公共 API 导出。由 `packages/scone-ui/src/index.test.ts` 和 build types 输出边界守护。
- `SCONE_RECIPE_DOCS_ONLY`：Recipe 只作为文档说明，不新增 `packages/scone-ui/src/recipes/`，也不导出 `SconeDrawerForm`、`SconePopover`、`SconeLogo`、`SconeResult`、`SconeGrid` 等 recipe 名称。由 `packages/scone-ui/src/index.test.ts` 守护。

### Testing

- `SCONE_TEST_FILTER_NOISY_STDERR`：测试输出使用 Vitest 默认 reporter；已知由测试环境触发且暂不影响断言质量的 stderr 噪声由 `vitest.config.ts` 的 `onConsoleLog` 精确过滤，避免用完全静音掩盖新的诊断信息。
...
- [ ] 是否没有触碰 `packages/scone-ui/src/components/ui/` vendored primitive，除非任务明确要求更新 vendored 基础组件。
