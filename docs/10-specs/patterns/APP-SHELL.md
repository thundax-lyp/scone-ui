...
| `AppShell.Sidebar` | `collapsed`、`defaultCollapsed`、`children` | 只表达展示折叠状态，不内置 toggle 或 change 事件。   |
| `AppShell.Header`  | `children`、`actions`、`className`          | 全局操作区域。                                       |
| `AppShell.Main`    | `children`、`className`                     | 页面入口和收缩边界。                                 |
| `AppShell.Aside`   | `children`、`open`、`defaultOpen`           | 只表达辅助栏显示状态，不内置 toggle 或 change 事件。 |
