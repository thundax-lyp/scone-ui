...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Admin Pattern exports | `AppShell.Root/Sidebar/Header/Main/Aside` | `src/patterns/app-shell.tsx` | Pattern parts | 是 | `pattern-only` | `src/patterns/app-shell.tsx` | `src/patterns/app-shell.test.tsx` |
...
| Admin Pattern exports | `FilterBar.Root/Search/Fields/Actions/Summary` | `src/patterns/filter-bar.tsx` | Pattern parts | 是 | `pattern-only` | `src/patterns/filter-bar.tsx` | `src/patterns/filter-bar.test.tsx` |
...
| Data display | `SconeCard` | `src/components/data-display/card.tsx` | `Scone*` export | 否 | `scone-wrapper` | `src/components/data-display/card.tsx` | `src/components/data-display/card.test.tsx` |
| Data display | `SconeTag`、`SconeBadge` | `src/components/data-display/tag.tsx`、`src/components/data-display/badge.tsx` | `Scone*` export | 否 | `custom` | 各组件文件；`SconeTone` 来自 `src/types/foundation.ts` | 同目录同名 `*.test.tsx` |
| Navigation and media | `SconeBreadcrumb`、`SconeTabs`、`SconeDropdown`、`SconeMenu` | `src/components/navigation/breadcrumb.tsx`、`src/components/navigation/tabs.tsx`、`src/components/navigation/dropdown.tsx`、`src/components/navigation/menu.tsx` | `Scone*` export | 支持 | `vendored-shadcn` | 各组件文件；共享词表来自 `src/types/foundation.ts` | 同目录同名 `*.test.tsx` |
| Navigation and media | `SconeSegmented`、`SconeAccordion`、`SconeCollapsible` | `src/components/navigation/segmented.tsx`、`src/components/navigation/accordion.tsx`、`src/components/navigation/collapsible.tsx` | `Scone*` export | 支持 | `vendored-shadcn` | 各组件文件；共享词表来自 `src/types/foundation.ts` | 同目录同名 `*.test.tsx` |
| Navigation and media | `SconePagination`、`SconeTree` | `src/components/navigation/pagination.tsx`、`src/components/navigation/tree.tsx` | `Scone*` export | 否 | `custom` | 各组件文件；共享词表来自 `src/types/foundation.ts` | 同目录同名 `*.test.tsx` |
...
| Input / Search / Password / TextArea | `docs/10-specs/components/form/SCONE-INPUT.md`、`SCONE-SEARCH-INPUT.md`、`SCONE-PASSWORD-INPUT.md`、`SCONE-TEXTAREA.md` | Component | `scone-wrapper` | `SconeInput`、`SconeSearchInput`、`SconePasswordInput`、`SconeTextArea` | `src/components/form/input.tsx`、`search-input.tsx`、`password-input.tsx`、`textarea.tsx` | 各组件文件 | disabled / readOnly / invalid via Field | label 关联、readOnly/disabled、Field invalid | Component Family Designs / Form |
| Select | `docs/10-specs/components/form/SCONE-SELECT.md` | Component | `scone-wrapper` | `SconeSelect` | `src/components/form/select.tsx` | `src/components/form/select.tsx`；`SconeOption` 来自 `src/types/foundation.ts` | disabled / readOnly / invalid via Field | value/open 受控、Radix 键盘、aria | Component Family Designs / Form |
...
| Form helpers | `docs/10-specs/components/form/SCONE-FIELD-GROUP.md`、`SCONE-FORM-SECTION.md`、`SCONE-FORM-ACTIONS.md` | Pattern helper | `pattern-only` | `SconeFieldGroup`、`SconeFormSection`、`SconeFormActions` | `src/components/form/field-group.tsx`、`form-section.tsx`、`form-actions.tsx` | 各组件文件 | sticky / section / grouping | sticky/actions 布局与业务边界 | Component Family Designs / Form |
| Stack / Inline / Compact / Toolbar | `docs/10-specs/components/layout/SCONE-STACK.md`、`SCONE-INLINE.md`、`SCONE-COMPACT.md`、`SCONE-TOOLBAR.md` | Layout | `custom` | `SconeStack`、`SconeInline`、`SconeCompact`、`SconeToolbar` | `src/components/layout/stack.tsx`、`inline.tsx`、`compact.tsx`、`toolbar.tsx` | 各组件文件；spacing/density 类型来自 `src/types/foundation.ts` | 不承载业务状态 | density、gap、wrap、ref/className | Component Family Designs / Layout |
...
| AppShell / Page / Section / FilterBar / DataTable | `docs/10-specs/patterns/APP-SHELL.md`、`PAGE.md`、`SECTION.md`、`FILTER-BAR.md`、`DATA-TABLE.md` | Admin Pattern | `pattern-only` | `AppShell.*`、`Page.*`、`Section.*`、`FilterBar.*`、`DataTable.*` | `src/patterns/app-shell.tsx`、`page.tsx`、`section.tsx`、`filter-bar.tsx`、`data-table.tsx` | 各 Pattern 文件 | data / sticky / selection 按 Pattern | slot、状态归属、滚动/sticky、业务边界 | Admin Pattern Designs |
| Table | `docs/10-specs/components/data-display/SCONE-TABLE.md` | Component | `scone-wrapper` | `SconeTable` | `src/components/data-display/table.tsx` | `src/components/data-display/table.tsx`；共享词表来自 `src/types/foundation.ts` | loading / empty / error | 状态优先级、columns、scroll、ref/className | Component Family Designs / Data Display |
| Pagination | `docs/10-specs/components/navigation/SCONE-PAGINATION.md` | Component | `custom` | `SconePagination` | `src/components/navigation/pagination.tsx` | `src/components/navigation/pagination.tsx` | disabled | page/pageSize、ARIA、键盘 | Component Family Designs / Navigation |
| Drawer | `docs/10-specs/components/feedback-overlay/SCONE-DRAWER.md` | Component | `scone-wrapper` | `SconeDrawer` | `src/components/feedback-overlay/drawer.tsx` | `src/components/feedback-overlay/drawer.tsx`；`OverlayCloseReason` 来自 `src/types/foundation.ts` | loading / close reason | focus trap/restore、close reason、widthPreset | Component Family Designs / Feedback |
| Card / Alert / Empty / Loading / Progress | `docs/10-specs/components/data-display/SCONE-CARD.md`、`feedback-overlay/SCONE-ALERT.md`、`SCONE-EMPTY.md`、`SCONE-LOADING.md`、`SCONE-PROGRESS.md` | Component | `scone-wrapper` | `SconeCard`、`SconeAlert`、`SconeEmpty`、`SconeLoading`、`SconeProgress` | `src/components/data-display/card.tsx`、`src/components/feedback-overlay/alert.tsx`、`empty.tsx`、`loading.tsx`、`progress.tsx` | 各组件文件；tone/status 类型来自 `src/types/foundation.ts` | loading / empty / error | tone/status、aria-busy、action | Component Family Designs |
| Descriptions / Tag / Badge / List / Typography / Statistic | `docs/10-specs/components/data-display/SCONE-DESCRIPTIONS.md`、`SCONE-TAG.md`、`SCONE-BADGE.md`、`SCONE-LIST.md`、`SCONE-TYPOGRAPHY.md`、`SCONE-STATISTIC.md` | Component | `custom` | `SconeDescriptions`、`SconeTag`、`SconeBadge`、`SconeList`、`SconeTypography`、`SconeText`、`SconeTitle`、`SconeParagraph`、`SconeStatistic` | `src/components/data-display/descriptions.tsx`、`tag.tsx`、`badge.tsx`、`list.tsx`、`typography.tsx`、`statistic.tsx` | 各组件文件；共享词表来自 `src/types/foundation.ts` | empty / loading 按组件适用 | density、tone、typography、item 类型 | Component Family Designs / Data Display |
...
| RadioGroup / Slider | `docs/10-specs/components/form/SCONE-RADIO-GROUP.md`、`SCONE-SLIDER.md` | Component | `vendored-shadcn` | `SconeRadioGroup`、`SconeSlider` | `src/components/form/radio-group.tsx`、`slider.tsx` | 各组件文件；`SconeOption` 来自 `src/types/foundation.ts` | selected / disabled / invalid via Field | keyboard、value、aria | Component Family Designs / Form |
| NumberInput / DatePicker / Upload | `docs/10-specs/components/form/SCONE-NUMBER-INPUT.md`、`SCONE-DATE-PICKER.md`、`SCONE-UPLOAD.md` | Component | `custom` | `SconeNumberInput`、`SconeDatePicker`、`SconeUpload` | `src/components/form/number-input.tsx`、`date-picker.tsx`、`upload.tsx` | 各组件文件 | disabled / readOnly / invalid via Field | custom 键盘、ARIA、事件 payload | Component Family Designs / Form |
| Combobox / Command | `docs/10-specs/components/form/SCONE-COMBOBOX.md`、`navigation/SCONE-COMMAND.md` | Component | `scone-wrapper` | `SconeCombobox`、`SconeCommand` | `src/components/form/combobox.tsx`、`src/components/navigation/command.tsx` | 各组件文件；`SconeOption`/`SconeBaseItem` 来自 `src/types/foundation.ts` | loading / empty / selected / expanded | search、selection、keyboard、empty | Component Family Designs |
| Breadcrumb / Tabs / Dropdown / Menu | `docs/10-specs/components/navigation/SCONE-BREADCRUMB.md`、`SCONE-TABS.md`、`SCONE-DROPDOWN.md`、`SCONE-MENU.md` | Component | `vendored-shadcn` | `SconeBreadcrumb`、`SconeTabs`、`SconeDropdown`、`SconeMenu` | `src/components/navigation/breadcrumb.tsx`、`tabs.tsx`、`dropdown.tsx`、`menu.tsx` | 各组件文件；`SconeBaseItem` 来自 `src/types/foundation.ts` | selected / expanded | aria-current、roving focus、typeahead | Component Family Designs / Navigation |
| Segmented / Accordion / Collapsible | `docs/10-specs/components/navigation/SCONE-SEGMENTED.md`、`SCONE-ACCORDION.md`、`SCONE-COLLAPSIBLE.md` | Component | `vendored-shadcn` | `SconeSegmented`、`SconeAccordion`、`SconeCollapsible` | `src/components/navigation/segmented.tsx`、`accordion.tsx`、`collapsible.tsx` | 各组件文件；共享词表来自 `src/types/foundation.ts` | selected / expanded | value/open、keyboard、ARIA | Component Family Designs / Navigation |
| Tree | `docs/10-specs/components/navigation/SCONE-TREE.md` | Component | `custom` | `SconeTree` | `src/components/navigation/tree.tsx` | `src/components/navigation/tree.tsx`；`Key` 来自 `src/types/foundation.ts` | selected / checked / expanded | tree keyboard、ARIA、selection/checking | Component Family Designs / Navigation |
| Image / Avatar / Tooltip | `docs/10-specs/components/media/SCONE-IMAGE.md`、`SCONE-AVATAR.md`、`docs/10-specs/components/navigation/SCONE-TOOLTIP.md` | Component | `vendored-shadcn` | `SconeImage`、`SconeAvatar`、`SconeTooltip` | `src/components/media/image.tsx`、`src/components/media/avatar.tsx`、`src/components/navigation/tooltip.tsx` | 各组件文件 | fallback / hover / focus | fallback、alt、tooltip trigger/focus | Component Family Designs / Media/Navigation |
| Dialog / Confirm | `docs/10-specs/components/feedback-overlay/SCONE-DIALOG.md`、`SCONE-CONFIRM.md` | Component | `scone-wrapper` | `SconeDialog`、`SconeConfirm` | `src/components/feedback-overlay/dialog.tsx`、`confirm.tsx` | 各组件文件；`OverlayCloseReason` 来自 `src/types/foundation.ts` | close reason / loading / destructive | focus trap、close reason、destructive action | Component Family Designs / Feedback |
...
| Timeline | `docs/10-specs/components/data-display/SCONE-TIMELINE.md` | Component | `custom` | `SconeTimeline` | `src/components/data-display/timeline.tsx` | `src/components/data-display/timeline.tsx`；`Key`/`SconeTone` 来自 `src/types/foundation.ts` | event sequence | item sequence、tone、accessibility | Component Family Designs / Data Display |
...
- SPEC 新增或排除能力时，必须同步本矩阵和 `docs/10-specs/COMPONENT-SELECTION.md`。
- 单组件 SPEC 调整 source strategy 时，以单组件 SPEC 和 `COMPONENT-SELECTION.md` 更新后的交集为准。
