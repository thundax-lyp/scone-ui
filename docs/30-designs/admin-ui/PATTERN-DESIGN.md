...
- Pattern parts 从 `src/patterns/index.ts` 和 `packages/scone-ui/src/index.ts` 导出；内部 helper 不导出。
...
以下 props contract 定义在各 Pattern 目标文件中，并从 `src/patterns/index.ts` 与 `packages/scone-ui/src/index.ts` 公开导出。Pattern props 只表达布局、slot、UI 状态和调用方意图，不接收请求函数、权限表达式、router 对象或业务 schema。
...
- 公共汇总：`src/patterns/index.ts`、`packages/scone-ui/src/index.ts`。
