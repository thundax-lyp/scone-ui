...
| DataTable | `src/patterns/data-table.tsx` | `DataTable.Root/FilterBar/Toolbar/BulkActions/TableRegion/Pagination` | sorting/filtering/pagination/selection/columnVisibility 由调用方或 TanStack recipe 拥有 | TableRegion、loading/error/empty、pagination 唯一入口 |
...
- sorting、filtering、pagination、selection、column visibility 由调用方或 TanStack recipe 拥有。
...
| --- | --- | --- | --- | --- | --- |
| DrawerForm | `SconeDrawer`、`SconeForm`、`SconeFormSection`、`SconeFormActions`、`SconeConfirm` | `src/recipes/drawer-form.tsx` 或 docs-only 示例 | 待审核 | 是组合流程，不是通用任务容器 API | 文档结构或 recipe 示例验证 |
| ConfirmationFlow | `SconeConfirm`、AlertDialog、Button | `src/recipes/confirmation-flow.tsx` 或 docs-only 示例 | 待审核 | 危险确认由组件组合表达，业务动作由调用方处理 | Confirm 行为和文案结构验证 |
...
| Dashboard Metric | `SconeStatistic`、Card、Grid recipe | `src/recipes/dashboard-metric.tsx` 或 docs-only 示例 | 待审核 | 指标业务口径由产品侧组合 | 文档结构或 recipe 示例验证 |
| Grid | CSS grid/Tailwind utilities、Page/Section/Card | `src/recipes/grid.tsx` 或 docs-only 示例 | 待审核 | 布局 recipe 不需要独立 `SconeGrid` | 响应式列数和 token 间距验证 |
